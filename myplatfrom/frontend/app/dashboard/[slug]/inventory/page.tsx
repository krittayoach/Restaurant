  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display font-bold text-2xl text-text">คลังวัตถุดิบ</h1>
          <p className="text-muted text-sm mt-0.5">{ingredients.length} รายการ</p>
        </div>
      ...
      {lowStock.length > 0 && (
        <div className="card p-4 mb-5 border-rose/30 bg-rose/5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-semibold text-rose">วัตถุดิบใกล้หมด ({lowStock.length} รายการ)</p>
          </div>
          <div className="space-y-1.5">
            {lowStock.map(i => (
              <div key={i.id} className="flex items-center justify-between text-sm">
                <span className="text-text">{i.name}</span>
                <span className="text-rose font-semibold">{i.quantity} {i.unit} (ต่ำกว่า {i.low_threshold})</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {showAdd && (
        <div className="card p-5 mb-5">
          <div className="flex items-center justify-between mb-4">
            <p className="font-semibold text-text">เพิ่มวัตถุดิบใหม่</p>
          ...
          <form onSubmit={addIngredient} className="grid grid-cols-2 gap-3">
            <div className="col-span-2 md:col-span-1">
              <label className="text-xs text-muted mb-1 block">ชื่อวัตถุดิบ *</label>
            ...
            <div>
              <label className="text-xs text-muted mb-1 block">จำนวนเริ่มต้น</label>
            ...
            <div>
              <label className="text-xs text-muted mb-1 block">แจ้งเตือนเมื่อต่ำกว่า</label>
      ...
      <div className="card overflow-hidden mb-8">
        <table className="w-full text-sm">
          <thead className="bg-bg3 text-muted text-xs">
            <tr>
              <th className="text-left px-4 py-3">วัตถุดิบ</th>
              <th className="text-right px-4 py-3">คงเหลือ</th>
              <th className="text-right px-4 py-3">แจ้งเตือน</th>
              <th className="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {ingredients.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-muted">ยังไม่มีวัตถุดิบ</td></tr>
            )}
            {ingredients.map(ing => {
      ...
      <div>
        <h2 className="font-display font-bold text-lg text-text mb-4 flex items-center gap-2">
          <Package size={18} className="text-accent" /> Recipe (วัตถุดิบต่อเมนู)
        </h2>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 border-t border-border">
                    {recipeMenuId === menu.id ? (
                      ...
                      <div className="pt-3">
                        {recipe.length === 0
                          ? <p className="text-muted text-sm">ยังไม่มี recipe</p>
